...
function AdminProdutosPage() {
  ...
  async function fetchProdutos(reset = false) {
    ...
    try {
      ...
      const aplicarPrecoNoCliente = !(
        precoMinNum !== undefined &&
        !isNaN(precoMinNum) &&
        precoMaxNum !== undefined &&
        !isNaN(precoMaxNum)
      );
  ...
  async function handleDelete(id: string) {
    if (
      !window.confirm(
        "Excluir produto de forma permanente? Essa ação não pode ser desfeita.",
      )
    )
      return;
  ...
  async function handleStatus(
    ...
    setProdutos((ps) =>
      ps.map((p) => (p.id === id ? { ...p, status: novo } : p)),
    );
    await logAdminAction("update-status-produto", id, before || null, {
      status: novo,
    });
  }
  ...
  async function handleApplyTag(id: string, tag: string) {
    ...
    setProdutos((ps) =>
      ps.map((x) => (x.id === id ? { ...x, tags: Array.from(tags) } : x)),
    );
  ...
  async function bulkStatus(novo: "Ativo" | "Bloqueado" | "Inativo") {
    ...
    if (
      !window.confirm(
        `Alterar status de ${idsSelecionados.length} produto(s) para ${novo}?`,
      )
    )
      return;
    await Promise.all(
  ...
  async function bulkTag(tag: string) {
    ...
    if (
      !window.confirm(
        `Aplicar a tag "${tag}" em ${idsSelecionados.length} produto(s)?`,
      )
    )
      return;
    await Promise.all(
                ...
                >
                  ...
                  >
                    ...
                    {asStatus(p) !== "Bloqueado" ? (
                      <button
                        onClick={() => handleStatus(p.id, "Bloqueado")}
                        style={btnDanger()}
                      >
                      ...
                      <button
                        onClick={() => handleStatus(p.id, "Ativo")}
                        style={btnSuccess()}
                      >
