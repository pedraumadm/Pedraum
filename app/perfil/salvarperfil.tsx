...
import TaxonomyQuickSearch, {
  TaxonomyPath,
} from "@/components/TaxonomyQuickSearch";
...
export default function PerfilPage() {
  ...
  const nomesCategoriasTodos = useMemo(
  ...
  useEffect(() => {
    const unsubAuth = auth.onAuthStateChanged((user) => {
      ...
      const unsubUser = onSnapshot(ref, (snap) => {
        ...
        const pairs: CategoriaPair[] = Array.isArray(
        ...
        if (
          !pairs?.length &&
          Array.isArray(data.categoriasAtuacao) &&
          data.categoriasAtuacao.length
        ) {
        ...
        const triplets: CategoriaTriplet[] = Array.isArray(
  ...
  function onPickTaxonomy(path: TaxonomyPath) {
    ...
    if (c1 && c2 && c3) {
      ...
      if (categoriasLocked && isCategoriaNova) {
        setMsg(
          "Categorias travadas: adicione itens apenas das categorias já escolhidas.",
        );
        return;
      }
      if (
        ...
        setMsg(
          `Você já tem ${MAX_CATEGORIAS}/${MAX_CATEGORIAS} categorias. Use uma existente.`,
        );
        return;
      }
  ...
  function dedupPairs(pairs: CategoriaPair[]) {
    ...
    for (const p of pairs)
      m.set(
        `${p.categoria.trim().toLowerCase()}::${p.subcategoria.trim().toLowerCase()}`,
        p,
      );
  ...
  function dedupTriplets(tris: CategoriaTriplet[]) {
    ...
    for (const t of tris)
      m.set(
        `${t.categoria.trim().toLowerCase()}::${t.subcategoria.trim().toLowerCase()}::${t.item.trim().toLowerCase()}`,
        t,
      );
    return Array.from(m.values());
  }
  ...
  const selectedCategorias = useMemo(
    () => Array.from(selectedCategoriasSet),
    [selectedCategoriasSet],
  );
  ...
  const categoriasDropdownBase = useMemo(() => {
    ...
    if (categoriasLocked)
      return base.filter((c) => selectedCategoriasSet.has(c));
    if (selectedCategoriasSet.size >= MAX_CATEGORIAS)
      return base.filter((c) => selectedCategoriasSet.has(c));
    return base;
  ...
  const itensDaSubSelecionada = useMemo(() => {
    ...
    const cat = categorias.find((c) => c.nome === selCategoria);
    const sub = (cat?.subcategorias || []).find(
      (s: any) => (typeof s === "string" ? s : s?.nome) === selSubcat,
    );
  ...
  function addTripletsSelecionados() {
    ...
    if (!selItens.length) {
      setMsg("Marque um ou mais itens.");
      return;
    }
  ...
  function addItemManual() {
    ...
    if (!nome) {
      setMsg("Digite o nome do item.");
      return;
    }
    if (!selectedCategoriasSet.has(selCategoria) && categoriasLocked) {
      setMsg(
        "Categorias travadas: adicione dentro das categorias já escolhidas.",
      );
      return;
    }
  ...
  function removeParCategoria(par: CategoriaPair) {
    setForm((f) => {
      const futuros = f.categoriasAtuacaoPairs.filter(
        (p) =>
          !(
            p.categoria === par.categoria && p.subcategoria === par.subcategoria
          ),
      );
      if (categoriasLocked) {
        const aindaTemDaCategoria =
          futuros.some((p) => p.categoria === par.categoria) ||
          (f.categoriasAtuacaoTriplets || []).some(
            (t) => t.categoria === par.categoria,
          );
        if (!aindaTemDaCategoria) {
          setMsg(
            "Categorias travadas: não é possível remover a última subcategoria de uma categoria.",
          );
          return f;
        }
      }
  ...
  async function pedirAlteracaoViaWhatsApp() {
    ...
    try {
      await addDoc(collection(db, "supportRequests"), {
        ...
        mensagem:
          "Solicito alteração nas minhas CATEGORIAS de atuação (não subcategorias/itens).",
    ...
    const pairsTxt = (form.categoriasAtuacaoPairs || [])
      ...
      .join(" | ");
    const tripTxt = (form.categoriasAtuacaoTriplets || [])
  ...
  async function salvar(e?: React.FormEvent) {
    ...
    try {
      ...
      if (categoriasDistintas.length > MAX_CATEGORIAS) {
        setMsg(
          `Você pode escolher no máximo ${MAX_CATEGORIAS} categorias distintas.`,
        );
        setSaving(false);
        return;
      }
      ...
      if (categoriasLocked) {
        ...
        if (!mesmas) {
          setMsg(
            "Categorias travadas: não é possível alterar o conjunto de CATEGORIAS.",
          );
          setSaving(false);
          return;
        }
      ...
      const shouldLockNow =
        !categoriasLocked && categoriasDistintas.length === MAX_CATEGORIAS;

      const categoriesAll = buildCategoriesAll(
        paresDedup,
        form.categoriasAtuacao,
        triplets,
      );
    ...
    >
      ...
      {categoriasLocked && (
        <div className="lock-banner">
          <Lock size={16} />
          Suas <b>CATEGORIAS</b> estão travadas. Você ainda pode gerenciar{" "}
          <b>subcategorias</b> e <b>itens</b> dentro delas.
          <button
            type="button"
            className="btn-sec"
      ...
      <form onSubmit={salvar} className="grid gap-16">
        ...
        <div className="card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
            ...
            <div>
              <div className="label">
                Categorias (até {MAX_CATEGORIAS} <b>categorias</b>;{" "}
                <b>subcategorias</b> e <b>itens</b> ilimitados)
              </div>
                  ...
                  <>
                    ...
                    >
                      ...
                      <button
                        ...
                        className="btn-sec"
                        disabled={
                          !selCategoria || !selSubcat || !selItens.length
                        }
                  ...
                  >
                    ...
                    <div className="grid gap-2">
                      ...
                      <button
                        type="button"
                        className="btn-sec"
                        onClick={usarSubcategoriaComoItem}
                      >
              ...
              {selectedCategorias.length > 0 && (
                <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
                  {selectedCategorias.map((cat) => {
                    const paresDaCat = form.categoriasAtuacaoPairs.filter(
                      (p) => p.categoria === cat,
                    );
                    const itensDaCat = (
                      form.categoriasAtuacaoTriplets || []
                          ...
                          <>
                            ...
                            <div className="chips">
                              {paresDaCat.map((p, idx) =>
                                p.subcategoria ? (
                                  <span
                                    key={`${p.categoria}__${p.subcategoria}__${idx}`}
                                    className="chip"
                                  >
                                    <Tag size={14} /> {p.subcategoria}
                                    <button
                                      type="button"
                                      onClick={() => removeParCategoria(p)}
                                    >
                                      ×
                                    </button>
                                  </span>
                          ...
                          >
                            {(() => {
                              ...
                              itensDaCat.forEach((t) => {
                                if (!mapSub.has(t.subcategoria))
                                  mapSub.set(t.subcategoria, []);
                                mapSub.get(t.subcategoria)!.push(t.item);
                              });
                              return Array.from(mapSub.entries()).map(
                                ([sub, itens]) => (
                                  <div
                                    key={`sub-${cat}-${sub}`}
                                    style={{ paddingLeft: 2 }}
                                  >
                                    ...
                                    <div className="chips">
                                      {itens.map((item, iidx) => (
                                        <span
                                          key={`${cat}__${sub}__${item}__${iidx}`}
                                          className="chip"
                                        >
                                          <Tag size={14} /> {item}
                                          <button
              ...
              {categoriasLocked ? (
                ...
                >
                  <span style={{ fontWeight: 800 }}>
                    ...
                    />
                    Conjunto de CATEGORIAS travado (subcategorias/itens
                    liberados)
                  </span>
                  <button
                    type="button"
                ...
                <div style={{ marginTop: 8, fontSize: 12, color: "#334155" }}>
                  ...
                  categorias. A partir de agora, selecione apenas{" "}
                  <b>subcategorias</b> e <b>itens</b> dessas categorias.
                </div>
              ) : (
                <div style={{ marginTop: 8, fontSize: 12, color: "#334155" }}>
                  Ao salvar com <b>{MAX_CATEGORIAS} categorias</b>, o conjunto
                  de categorias ficará travado (subcategorias/itens continuam
                  livres).
                </div>
              )}
            </div>
              ...
              >
                ...
                <div className="px-4 pb-4">
                  ...
                  <p className="text-xs text-slate-500 mt-2">
                    Envie JPG/PNG. Dica: priorize trabalhos finalizados,
                    antes/depois, certificados etc.
                  </p>
                </div>
              </div>
                ...
                <div className="px-4 pb-4 space-y-3">
                  ...
                  {pdfUrl ? (
                    ...
                    <p className="text-xs text-slate-500">
                      Anexe seu portfólio consolidado, certificações ou
                      catálogos (até 8MB).
                    </p>
                  )}
                </div>
